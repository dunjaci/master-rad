...
Posebno interesantan problem predstavlja određivanje početnog regiona replikacije (\textit{oriC}), koji ima ključnu ulogu u procesu umnožavanja genetskog materijala. U okviru velikih genoma, koji mogu sadržati milione nukleotida, ovaj region je relativno kratak i nije eksplicitno označen. Stoga se njegovo identifikovanje zasniva na analizi obrazaca u DNK sekvenci i prepoznavanju karakterističnih signala koji ukazuju na početak replikacije.
...
Početni region replikacije, označen kao \textit{oriC}, predstavlja specifičan segment DNK u kojem započinje proces replikacije. Kod bakterija, genom je najčešće organizovan u obliku kružnog hromozoma i sadrži samo jedan takav region iz kojeg se replikacija širi u oba smera, kao što je prikazano na Slici \ref{fig:oric}.
...
\label{fig:oric}
\end{figure}
Ovaj region sadrži sekvence koje omogućavaju vezivanje proteina neophodnih za početak replikacije i time ima ključnu ulogu u pokretanju celokupnog procesa umnožavanja genetskog materijala. Iako je \textit{oriC} relativno kratak u poređenju sa dužinom celog genoma, koji može sadržati milione nukleotida, on u DNK sekvenci nije eksplicitno označen. Zbog toga, njegovo pronalaženje predstavlja složen i netrivijalan zadatak.
...
Nakon vezivanja \textit{DnkA} proteina dolazi do lokalnog razdvajanja DNK lanca, čime se omogućava delovanje \textit{DNK polimeraze}, enzima koji funkcioniše kao molekularna mašina za kopiranje genetskog materijala i vrši sintezu novih DNK lanaca.
...
Podniske dužine $k$ jedne biološke sekvence (DNK, njenog segmenta ili proteina) nazivaju se $k$-grami ili $k$-meri i predstavljaju osnovne jedinice analize bioloških sekvenci. Analizom frekvencije njihovog pojavljivanja moguće je identifikovati obrasce koji mogu ukazivati na prisustvo funkcionalno značajnih regiona, kao što je \textit{oriC}. 
...
Funkcija \textit{\textbf{PatternToNumber}} preslikava dati $k$-gram u odgovarajući indeks tako što njegov prefiks tretira kao već izračunatu vrednost, koju zatim množi sa 4 i dodaje vrednost poslednjeg simbola.  Ovaj postupak odgovara standardnoj konverziji iz pozicionog brojčanog sistema i omogućava da se svaki $k$-gram jednoznačno predstavi celobrojnim indeksom. Algoritam \ref{alg:pattern_to_number} prikazuje pseudokod ove funkcije.
...
Nakon što je niz frekvencija izračunat, najčešći $k$-grami se dobijaju pronalaženjem maksimalne vrednosti u nizu i identifikacijom svih indeksa koji imaju tu vrednost. Algoritam \ref{alg:faster_frequent_words} prikazuje pseudokod funkcije \textit{Faster Frequent Words}.
...
Međutim, dodatni trošak predstavlja inicijalizacija niza dimenzije $4^k$, kao i iteracija kroz ceo niz prilikom traženja maksimuma i rekonstrukcije odgovarajućih $k$-grama. Ukupna složenost algoritma iznosi:
...
Za razliku od pristupa sa nizom frekvencija, ovde nije potrebno inicijalizovati niz dimenzije $4^k$, niti prolaziti kroz sve moguće $k$-grame. Pretražuju se samo oni obrasci koji su se stvarno pojavili u tekstu.
...
Neka je data niska $Text$. Za svaku poziciju $i$ izdvajamo $k$-gram $Text[i..i+k-1]$ i transformišemo ga u odgovarajući broj korišćenjem funkcije \textit{\textbf{PatternToNumber}}. Na taj način dobijamo niz indeksa koji zatim sortiramo. Algoritam \ref{alg:ffw_sorting} prikazuje pseudokod ovog pristupa.

Nakon sortiranja, identični $k$-grami se pojavljuju u uzastopnim segmentima niza, što omogućava jednostavno prebrojavanje njihovih pojavljivanja.
...
Nakon sortiranja identični indeksi se grupišu jedan do drugog (Slika \ref{fig:sorted}).
\begin{figure}[h]
\centering
...
Proces replikacije DNK započinje u specifičnom regionu genoma poznatom kao \textit{oriC}, odakle se replikacija odvija u oba smera duž hromozoma, sve do regiona označenog kao \textit{terC}. Tokom ovog procesa formiraju se dve replikacione viljuške (\textit{replication forks}), duž kojih se replikacija odvija u suprotnim smerovima.

DNK polimeraza sintetiše novi lanac na osnovu postojećeg, ali može da dodaje nukleotide isključivo u smeru $5' \rightarrow 3'$.  Postavlja se pitanje kako se vrši replikacija u suprotnom smeru, $3' \rightarrow 5'$. Ovo ograničenje dovodi do asimetrije u procesu replikacije i predstavlja osnovu za kasnije nastajanje razlika u raspodeli nukleotida duž genoma.
...
Na osnovu smera replikacije, kružni bakterijski hromozom može se podeliti na dve polovine označene kao \textit{forward half-strand} i \textit{reverse half-strand}. Na \textit{reverse half-strand} polovini hromozoma DNK polimeraza može kontinuirano da prati otvaranje DNK molekula i sintetiše novi lanac bez prekida. Nasuprot tome, na \textit{forward half-strand} polovini hromozoma smer kretanja DNK polimeraze se razlikuje od orijentacije lanca, pa se sinteza ne može odvijati kontinualno.
...
Pošto DNK polimeraza ne može da sintetiše DNK u smeru $3' \rightarrow 5'$, na \textit{forward half-strand} polovini hromozoma neophodno je višestruko započinjanje sinteze. Svaki novi segment započinje formiranjem RNK prajmera, odnosno kratkog lanca RNK koji sintetiše enzim primaza i koji obezbeđuje početni $3'$ kraj sa koga DNK polimeraza može započeti sintezu. Ovi kratki DNK segmenti nazivaju se \textbf{Okazaki fragmenti}.

Kako se replikacioni viljuškasti region pomera, nastaju novi Okazaki fragmenti, a DNK polimeraza periodično započinje sintezu novih fragmenata. Po završetku replikacije, RNK prajmeri se uklanjaju, njihovo mesto popunjavaju DNK nukleotidi, a enzim DNK ligaza povezuje susedne fragmente u kontinuirani lanac. Zbog isprekidane sinteze, na \textit{forward half-strand} polovini hromozoma potreban je poseban RNK prajmer za svaki novonastali Okazaki fragment. Nasuprot tome, na \textit{reverse half-strand} polovini hromozoma sinteza se odvija kontinualno, pa je dovoljan samo jedan RNK prajmer \cite{alberts2022}. 

Na Slici \ref{fig:replikacija1} prikazana je asimetrija replikacije DNK na \textit{reverse} i \textit{forward half-strand} polovini hromozoma, dok Slika \ref{fig:asimetrija} prikazuje ilustraciju koraka.

\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{images/asimetrija_replikacije.png}
\caption{Asimetrija replikacije na reverse (leading) i forward (lagging) half-strand polovinama hromozoma \cite{leman2013}}
...
% Titles
\node[title] at (3.7,0) {Reverse half-strand};
\node[subtitle] at (3.7,-0.35) {kontinuirana sinteza};

\node[title] at (12.0,0) {Forward half-strand};
...
\caption{Uporedni prikaz asimetrije replikacije DNK na \textit{reverse half-strand} i \textit{forward half-strand} polovini hromozoma. Plavi segment označava roditeljski DNK lanac koji služi kao matrica, zeleni segmenti označavaju RNK prajmere, a crveni segmenti novosintetisanu DNK.}
...
Minimum funkcije dostiže se približno na poziciji od oko 4 miliona nukleotida i predstavlja kandidat za lokaciju početnog regiona replikacije (\textit{oriC}). Razlog za to je što upravo na tom mestu dolazi do promene između \textit{forward} i \textit{reverse half-strand}, odnosno do promene u raspodeli baza G i C.
...
Na primeru sa Slike \ref{fig:hamming_primer}, sekvence \texttt{ATCGTAGC} i \texttt{ATGGTCGC} razlikuju se u dve pozicije, pa je njihova Hamingova distanca jednaka 2.
...
Najjednostavniji način računanja Hamingove distance zasniva se na poređenju odgovarajućih karaktera u dve sekvence iste dužine. Svaki put kada se simboli razlikuju, brojač se uvećava za 1. Algoritam \ref{alg:hamming_distance} prikazuje pseudokod ove funckije.
...
\subsection{Približno poklapanje obrazaca (\textit{Approximate Pattern Matching})}

Na osnovu Hamingove distance definiše se problem približnog poklapanja obrazaca. Za razliku od tačnog poklapanja, gde se zahteva potpuno podudaranje sekvenci, ovde se dozvoljava ograničen broj odstupanja.
...
Najjednostavniji pristup rešavanju ovog problema zasniva se na kliznom prozoru. Prozor dužine $|Pattern|$ pomera se kroz sekvencu $Text$, a za svaku poziciju računa se Hamingova distanca između obrasca i odgovarajuće podniske. Algoritam \ref{alg:approximate_pattern_matching} prikazuje pseudokod ove funcije.
...
Približno poklapanje i približno prebrojavanje pojavljivanja predstavljaju osnovu za definisanje složenijeg problema. Sada cilj nije pronaći poklapanje sa datim obrascem, već pronaći najčešće obrasce kada su dozvoljena odstupanja.
...
Funkcija \texttt{Neighbors(Pattern, d)} može se implementirati rekurzivno. Osnovna ideja rekurzije zasniva se na tome da se prvo generišu susedi sufiksa obrasca, a zatim se u zavisnosti od njihove Hamingove distance dodaje odgovarajući simbol na početak.Na ovaj način se korak po korak generišu sve sekvence koje zadovoljavaju uslov da se razlikuju od početnog obrasca u najviše $d$ pozicija. Pseudokod prikazan je u Algoritmu \ref{alg:neighbors}.
...
Najveći izazov ovog pristupa jeste dimenzija $d$-susedstva. Za dati obrazac dužine $k$, broj suseda raste sa povećanjem broja dozvoljenih odstupanja $d$. Taj broj se može precizno izraziti formulom:
...
Glavna prednost ovog pristupa je smanjenje memorijske potrošnje, jer se izbegava korišćenje nizova dimenzije $4^k$. Umesto toga, čuvaju se samo oni obrasci koji su zaista relevantni kandidati.
...
Jedan od glavnih izazova jeste dimenzija prostora pretrage. Broj mogućih $k$-grama iznosi $4^k$, a uz dozvoljena odstupanja dodatno raste zbog generisanja $d$-susedstva.
...
\caption{Podešavanja za deploy serverske komponente na Render platformu}
...
Stranica Teorijske osnove, prikazana na Slici \ref{fig:teorijske_osnove}, uvodi biološke pojmove potrebne za razumevanje algoritama. Objašnjeni su struktura DNK molekula, proces replikacije, početni region replikacije, DnkA boksovi i pojam obrnutog komplementa. Klikom na sekciju prikazuju se interaktivne kartice sa detaljnim objašnjenjima  pojmova. Ovaj deo lekcije povezuje biološku motivaciju sa algoritamskim problemima koji se kasnije obrađuju.
...
Korisnik unosi DNK sekvencu, a aplikacija prikazuje konstrukciju niza vrednosti korak po korak. Vizuelni prikaz obuhvata trenutnu bazu, trenutnu vrednost funkcije, grafički prikaz \textit{GC-skew} dijagrama i označene pozicije minimuma (Slika \ref{fig:gc_skew}).
...
Kao što je prikazano na Slici \ref{fig:hamming}, za Hamingovu distancu korisnik unosi dve sekvence iste dužine, a aplikacija prikazuje broj pozicija na kojima se one razlikuju.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{images/hamming_visual.png}
\caption{Interaktivni prikaz računanja Hamingove distance za dve sekvence}
\label{fig:hamming}
\end{figure} 

Kod približnog poklapanja korisnik unosi tekst, obrazac i dozvoljeni broj odstupanja, nakon čega se prikazuju pozicije na kojima se obrazac pojavljuje uz najviše \(d\) razlika. Deo za d-susedstvo prikazuje skup svih sekvenci koje su od zadatog obrasca udaljene najviše \(d\) pozicija (Slika \ref{fig:d}).
